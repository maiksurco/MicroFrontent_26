...
const VentaProds = () => {
  ...
  const [filteredVentaProds, setFilteredVentaProds] = useState([]);
  ...
  const getCurrentItems = () => {
    ...
    return filteredVentaProds.slice(startIndex, endIndex);
  };
  ...
  const totalPages = Math.ceil(ventaProds.length / itemsPerPage);
  const hasNextPage = () => {
    return currentPage < totalPages && ventaProds.slice(currentPage * itemsPerPage, (currentPage + 1) * itemsPerPage).length > 0;
  };
  // end fracmento de paginacion
  const [ventaProdEditado, setVentaProdEditado] = useState({
    id: null,
    serie: "",
    fecha:"",
    descripcion:"",
  ...
  const getVentaProds = () => {
    axios
      ...
      .then((response) => {
        const sortedVentaProds = response.data.sort();
        setVentaProds(sortedVentaProds.reverse());
        const filtered = response.data.filter((ventaProd) => {
          const nombreCompleto = `${ventaProd.nombre}${ventaProd.descripcion}`;
          return nombreCompleto.toLowerCase().includes(searchTerm.toLowerCase());
        });

        setFilteredVentaProds(filtered);
      })
      .catch((error) => {
        // handle error
        console.log(error);
  ...
  const editarVentaProd = (id) => {
    const ventaProd = ventaProds.find((p) => p.id === id);
    setVentaProdEditado({
      id: ventaProd.id,
      serie: ventaProd.serie,
      fecha: ventaProd.fecha,
      descripcion: ventaProd.descripcion,
      metPago: ventaProd.metPago,
      estado: ventaProd.estado,
      proDes: ventaProd.proDes,
      total: ventaProd.total,
      socioId: ventaProd.socioId,
   
    });
    openModal()
  }

  const crearVentaProd = async (event) => {
    event.preventDefault();
    if (
      !ventaProdEditado.serie.trim() ||
      !ventaProdEditado.fecha.trim()||
      !ventaProdEditado.descripcion.trim()||
      !ventaProdEditado.metPago.trim()||
      !ventaProdEditado.estado.trim()||
      !ventaProdEditado.proDes.trim()||
      !ventaProdEditado.total.trim()||
      !ventaProdEditado.socioId.trim()
    ) {
      return;
    }
    ...
    try {
      ...
      const nuevoVentaProd = {
        serie: ventaProdEditado.serie,
        fecha: ventaProdEditado.fecha,
        descripcion: ventaProdEditado.descripcion,
        metPago: ventaProdEditado.metPago,
        estado: ventaProdEditado.estado,
        proDes: ventaProdEditado.proDes,
        total: ventaProdEditado.total,
        socioId: ventaProdEditado.socioId,

      };


      axios
        .post(`${API_URL}/ventas`, nuevoVentaProd,{
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
  ...
  const actualizarVentaProd = async (event) => {
    event.preventDefault();

    try {


      const ventaProdActualizado = {
        id: ventaProdEditado.id,
        nombre: ventaProdEditado.nombre,
        apellido: ventaProdEditado.apellido,
        edad: ventaProdEditado.edad,
        dni: ventaProdEditado.dni,
        numero: ventaProdEditado.numero,
        correo: ventaProdEditado.correo,
      };

      // Realizar la solicitud PUT para actualizar el socioProd
      const response = await axios.put(
        `${API_URL}/ventas`,
        ventaProdActualizado,{
          headers: {
            Authorization: `Bearer ${token}`,
          },
        }
      ...
      // Actualizar la lista de socioProds
      getVentaProds();
      closeModal()
      setVentaProdEditado({
        id: null,
  ...
  const contenidoModal = (
    ...
    <div className="px-6 py-6 mx-2 mt-2 mb-2 text-left bg-white   shadow-slate-400 shadow-md">
      ...
      <h2 className="text-2xl font-bold text-center">{ventaProdEditado.id ? 'Editar VentaProd' : 'Crear VentaProd'}</h2>
      <form onSubmit={ventaProdEditado.id ? actualizarVentaProd : crearVentaProd}>
        <div className="mt-4">
          <div className="flex flex-wrap">
            ...
            <div className="ml-1 w-full">
              ...
              <input
                value={ventaProdEditado.descripcion}
                onChange={(event) =>
                  setVentaProdEditado({
                    ...ventaProdEditado,
            ...
            <div className="ml-1 w-full">
              ...
              <input
                value={ventaProdEditado.metPago}
                onChange={(event) =>
                  setVentaProdEditado({
                    ...ventaProdEditado,
                    metPago: event.target.value,
                  })
                }
                type="text"
                placeholder="metPago"
            ...
            <div className="ml-1 w-full">
              <label className="block">Descuento de Producto</label>
              <input
                value={ventaProdEditado.proDes}
                onChange={(event) =>
                  setVentaProdEditado({
                    ...ventaProdEditado,
                    proDes: event.target.value,
                  })
                }
                type="text"
                placeholder="proDes"
          ...
          <div className="flex  flex-shrink-0 flex-wrap items-center justify-end rounded-b-md border-t-2 border-neutral-100 border-opacity-100 p-4 dark:border-opacity-50">
            ...
            >
              {ventaProdEditado.id ? (
                <button onClick={actualizarVentaProd} >Actualizar</button>
              ) : (
                <button onClick={crearVentaProd} >Crear </button>
              )}
            </div>
          </div>
        </div>
                  ...
                  <table class="items-center w-full mb-0 align-top border-collapse dark:border-white/40 text-slate-500">
                    <thead class="align-bottom">
                      <tr>
                        ...
                        <th class="px-6 py-3 pl-2 font-bold text-left uppercase align-middle bg-transparent border-b border-collapse shadow-none dark:border-white/40 dark:text-white text-xxs border-b-solid tracking-none whitespace-nowrap text-slate-400  ">Serie</th>
                        <th class="px-6 py-3 pl-2 font-bold text-left uppercase align-middle bg-transparent border-b border-collapse shadow-none dark:border-white/40 dark:text-white text-xxs border-b-solid tracking-none whitespace-nowrap text-slate-400  ">Fecha</th>
                        <th class="px-6 py-3 pl-2 font-bold text-left uppercase align-middle bg-transparent border-b border-collapse shadow-none dark:border-white/40 dark:text-white text-xxs border-b-solid tracking-none whitespace-nowrap text-slate-400  ">Descripción</th>
                        <th class="px-6 py-3 pl-2 font-bold text-left uppercase align-middle bg-transparent border-b border-collapse shadow-none dark:border-white/40 dark:text-white text-xxs border-b-solid tracking-none whitespace-nowrap text-slate-400  ">Metodo de Pago</th>
                        <th class="px-6 py-3 pl-2 font-bold text-left uppercase align-middle bg-transparent border-b border-collapse shadow-none dark:border-white/40 dark:text-white text-xxs border-b-solid tracking-none whitespace-nowrap text-slate-400  ">Estado</th>
                        <th class="px-6 py-3 pl-2 font-bold text-left uppercase align-middle bg-transparent border-b border-collapse shadow-none dark:border-white/40 dark:text-white text-xxs border-b-solid tracking-none whitespace-nowrap text-slate-400  ">Descuento de Producto</th>
                        <th class="px-6 py-3 pl-2 font-bold text-left uppercase align-middle bg-transparent border-b border-collapse shadow-none dark:border-white/40 dark:text-white text-xxs border-b-solid tracking-none whitespace-nowrap text-slate-400  ">Total</th>
                        <th class="px-6 py-3 pl-2 font-bold text-left uppercase align-middle bg-transparent border-b border-collapse shadow-none dark:border-white/40 dark:text-white text-xxs border-b-solid tracking-none whitespace-nowrap text-slate-400  ">Socio ID</th>
                    ...
                    <tbody>
                      {currentItems.map((ventaProd) => (
                        <tr key={ventaProd.id}>
                          ...
                          <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                            <p class="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{ventaProd.serie}</p>
                          </td>
                          <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                            <p class="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{ventaProd.fecha}</p>
                          </td>
                          <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                            <p class="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{ventaProd.descripcion}</p>
                          </td>
                          <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                            <p class="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{ventaProd.metPago}</p>
                          </td>
                          <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                            <p class="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{ventaProd.estado}</p>
                          </td>
                          <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                            <p class="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{ventaProd.proDes}</p>
                          </td>
                          <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                            <p class="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{ventaProd.total}</p>
                          </td>
                          <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                            <p class="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{ventaProd.socioId}</p>
                          </td>
                          <td class="p-2 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                            <span class="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 text-slate-400">{ventaProd.created_at}</span>
                          </td>
                          <td class="p-2 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                            <span class="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 text-slate-400">{ventaProd.updated_at}</span>
                          </td>
                          <td class=" sticky right-0 p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                            ...
                            <div className=" m-1 btns inline-block px-4 py-3 mb-0 font-bold text-center uppercase align-middle transition-all bg-transparent border-0 rounded-lg shadow-none cursor-pointer leading-pro text-xs ease-soft-in bg-150 hover:scale-102 active:opacity-85 bg-x-25" href="javascript:;">
                              <button onClick={() => eliminarVentaProd(ventaProd.id)} >
                            ...
                            <a class="btns inline-block px-4 py-3 mb-0 font-bold text-center uppercase align-middle transition-all bg-transparent border-0 rounded-lg shadow-none cursor-pointer leading-pro text-xs ease-soft-in bg-150 hover:scale-102 active:opacity-85 bg-x-25 " href="javascript:;">
                              <button onClick={() => editarVentaProd(ventaProd.id)} >
