...
const ServicioProds = () => {
  ...
  const [filteredServicioProds, setFilteredServicioProds] = useState([]);
  ...
  const getCurrentItems = () => {
    ...
    return filteredServicioProds.slice(startIndex, endIndex);
  };
  ...
  const [servicioProds, setServicioProds] = useState([]);
  //fracmento de paginacion
  const totalPages = Math.ceil(servicioProds.length / itemsPerPage);
  const hasNextPage = () => {
    return currentPage < totalPages && servicioProds.slice(currentPage * itemsPerPage, (currentPage + 1) * itemsPerPage).length > 0;
  };
  // end fracmento de paginacion
  const [servicioProdEditado, setServicioProdEditado] = useState({
    id: null,
    nombre: "",
    precio:"",
    duracion:"",
  ...
  const getServicioProds = () => {
    axios
      ...
      .then((response) => {
        const sortedServicioProds = response.data.sort();
        setServicioProds(sortedServicioProds.reverse());
        const filtered = response.data.filter((servicioProd) => {
          const nombreCompleto = `${servicioProd.nombre}${servicioProd.descripcion}`;
          return nombreCompleto.toLowerCase().includes(searchTerm.toLowerCase());
        });

        setFilteredServicioProds(filtered);
      })
      .catch((error) => {
        // handle error
        console.log(error);
  ...
  const editarServicioProd = (id) => {
    const servicioProd = servicioProds.find((p) => p.id === id);
    setServicioProdEditado({
      id: servicioProd.id,
      nombre: servicioProd.nombre,
      precio: servicioProd.precio,
      duracion: servicioProd.duracion,
      disponibilidad: servicioProd.disponibilidad,
      descripcion: servicioProd.descripcion,
   
    });
    openModal()
  }

  const crearServicioProd = async (event) => {
    event.preventDefault();
    if (
      !servicioProdEditado.nombre.trim() ||
      !servicioProdEditado.precio.trim()||
      !servicioProdEditado.duracion.trim()||
      !servicioProdEditado.disponibilidad.trim()||
      !servicioProdEditado.descripcion.trim()
    ) {
      return;
    }
    ...
    try {
      ...
      const nuevoServicioProd = {
        nombre: servicioProdEditado.nombre,
        precio: servicioProdEditado.precio,
        duracion: servicioProdEditado.duracion,
        disponibilidad: servicioProdEditado.disponibilidad,
        descripcion: servicioProdEditado.descripcion,

      };


      axios
        .post(`${API_URL}/servicios`, nuevoServicioProd,{
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
  ...
  const actualizarServicioProd = async (event) => {
    event.preventDefault();

    try {


      const servicioProdActualizado = {
        id: servicioProdEditado.id,
        nombre: servicioProdEditado.nombre,
        precio: servicioProdEditado.precio,
        duracion: servicioProdEditado.duracion,
        disponibilidad: servicioProdEditado.disponibilidad,
      ...
      // Realizar la solicitud PUT para actualizar el servicioProd
      const response = await axios.put(
        `${API_URL}/servicios`,
        servicioProdActualizado,{
          headers: {
            Authorization: `Bearer ${token}`,
          },
        }
      ...
      // Actualizar la lista de servicioProds
      getServicioProds();
      closeModal()
      setServicioProdEditado({
        id: null,
  ...
  const eliminarServicioProd = async (id) => {
    try {
      // Realizar la solicitud DELETE para eliminar el serviciosaProd
  ...
  const contenidoModal = (
    ...
    <div className="px-6 py-6 mx-2 mt-2 mb-2 text-left bg-white   shadow-slate-400 shadow-md">
      ...
      <h2 className="text-2xl font-bold text-center">{servicioProdEditado.id ? 'Editar ServicioProd' : 'Crear ServicioProd'}</h2>
      <form onSubmit={servicioProdEditado.id ? actualizarServicioProd : crearServicioProd}>
        <div className="mt-4">
          <div className="flex flex-wrap">
            ...
            <div className="ml-1 w-full">
              ...
              <input
                value={servicioProdEditado.nombre}
                onChange={(event) =>
                  setServicioProdEditado({
                    ...servicioProdEditado,
            ...
            <div className="ml-1 w-full">
              ...
              <input
                value={servicioProdEditado.precio}
                onChange={(event) =>
                  setServicioProdEditado({
                    ...servicioProdEditado,
            ...
            <div className="ml-1 w-full">
              ...
              <input
                value={servicioProdEditado.duracion}
                onChange={(event) =>
                  setServicioProdEditado({
                    ...servicioProdEditado,
            ...
            <div className="ml-1 w-full">
              <label className="block">Disponilibilidad </label>
              <input
                value={servicioProdEditado.disponibilidad}
                onChange={(event) =>
                  setServicioProdEditado({
                    ...servicioProdEditado,
            ...
            <div className="ml-1 w-full">
              ...
              <input
                value={servicioProdEditado.descripcion}
                onChange={(event) =>
                  setServicioProdEditado({
                    ...servicioProdEditado,
          ...
          <div className="flex  flex-shrink-0 flex-wrap items-center justify-end rounded-b-md border-t-2 border-neutral-100 border-opacity-100 p-4 dark:border-opacity-50">
            ...
            >
              {servicioProdEditado.id ? (
                <button onClick={actualizarServicioProd} >Actualizar</button>
              ) : (
                <button onClick={crearServicioProd} >Crear </button>
              )}
            </div>
          </div>
        </div>
          ...
          <div class="flex-none w-full max-w-full px-3">
            <div class=" flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
              <div class="p-6 pb-0 mb-0 border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
                <h6 class="dark:text-white font-bold">Lista De Servicios</h6>
              </div>
                        ...
                        <div class="flex items-center md:ml-auto md:pr-4">
                          <div class="relative flex flex-wrap items-stretch w-full transition-all rounded-lg ease-soft">
                            <span class="text-sm ease-soft absolute leading-5.6  z-50 -ml-px flex h-full items-center whitespace-nowrap rounded-lg rounded-tr-none rounded-br-none border border-r-0 border-transparent bg-transparent py-2 px-2.5 text-center font-normal text-slate-500 transition-all">
                              <button onClick={getServicioProds} >
                                <svg
                  ...
                  <table class="items-center w-full mb-0 align-top border-collapse dark:border-white/40 text-slate-500">
                    <thead class="align-bottom">
                      <tr>
                        ...
                        <th class="px-6 py-3 pl-2 font-bold text-left uppercase align-middle bg-transparent border-b border-collapse shadow-none dark:border-white/40 dark:text-white text-xxs border-b-solid tracking-none whitespace-nowrap text-slate-400  ">Duracion</th>
                        <th class="px-6 py-3 pl-2 font-bold text-left uppercase align-middle bg-transparent border-b border-collapse shadow-none dark:border-white/40 dark:text-white text-xxs border-b-solid tracking-none whitespace-nowrap text-slate-400  ">Disponibilidad</th>
                    ...
                    <tbody>
                      {currentItems.map((servicioProd) => (
                        <tr key={servicioProd.id}>
                          ...
                          <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                            <p class="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{servicioProd.nombre}</p>
                          </td>
                          <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                            <p class="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{servicioProd.precio}</p>
                          </td>
                          <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                            <p class="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{servicioProd.duracion}</p>
                          </td>
                          <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                            <p class="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{servicioProd.disponibilidad}</p>
                          </td>
                          <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                            <p class="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{servicioProd.descripcion}</p>
                          </td>
                          <td class="p-2 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                            <span class="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 text-slate-400">{servicioProd.created_at}</span>
                          </td>
                          <td class="p-2 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                            <span class="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 text-slate-400">{servicioProd.updated_at}</span>
                          </td>
                          <td class=" sticky right-0 p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                            ...
                            <div className=" m-1 btns inline-block px-4 py-3 mb-0 font-bold text-center uppercase align-middle transition-all bg-transparent border-0 rounded-lg shadow-none cursor-pointer leading-pro text-xs ease-soft-in bg-150 hover:scale-102 active:opacity-85 bg-x-25" href="javascript:;">
                              <button onClick={() => eliminarServicioProd(servicioProd.id)} >
                            ...
                            <a class="btns inline-block px-4 py-3 mb-0 font-bold text-center uppercase align-middle transition-all bg-transparent border-0 rounded-lg shadow-none cursor-pointer leading-pro text-xs ease-soft-in bg-150 hover:scale-102 active:opacity-85 bg-x-25 " href="javascript:;">
                              <button onClick={() => editarServicioProd(servicioProd.id)} >
